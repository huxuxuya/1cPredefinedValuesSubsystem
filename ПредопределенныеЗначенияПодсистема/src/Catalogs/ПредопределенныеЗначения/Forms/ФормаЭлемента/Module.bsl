...
Процедура ПередЗаписьюНаСервереСоставнойПараметр(ТекущийОбъект)
	...
	Иначе
		СтруктураХранилищаРанее = ПредопределенныеЗначения.ЗначениеСлужебнойНастройки(ТекущийОбъект.Ссылка);
	КонецЕсли;
...
Процедура УстановитьОграничениеТипа()
	...
	Описание = ПредопределенныеЗначения.ОписаниеНастройкиПоСсылке(Объект.Ссылка);
	Если Описание = Неопределено Или Не ЗначениеЗаполнено(Описание.ТипЗначения) Тогда
		Возврат;
	КонецЕсли;
	
	ТипЗначения = Описание.ТипЗначения;
	
	Если Объект.МногострочныйПараметр Тогда
		
		Элементы.ПараметрыЗначение.ОграничениеТипа	= Описание.ТипЗначения;
		Элементы.ПараметрыЗначение.ВыбиратьТип		= Ложь;
		
	Иначе
		
		Элементы.Параметр.ОграничениеТипа	= Описание.ТипЗначения;
		Элементы.Параметр.ВыбиратьТип		= Ложь;
