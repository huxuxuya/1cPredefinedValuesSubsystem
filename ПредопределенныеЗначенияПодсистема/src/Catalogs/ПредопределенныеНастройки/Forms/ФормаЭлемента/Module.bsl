...
Процедура ПереключитьСтраницу()
	...
	ИначеЕсли Объект.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.Булево") Тогда
...
Процедура УстановитьОформлениеЭлементаОтображенияЗначения()
	...
	Если ТипЗнч(Объект.Значение) = Тип("Булево") Тогда
		Элементы.Значение.Вид = ВидПоляФормы.ПолеФлажка;
		Элементы.Значение.ВидФлажка = ВидФлажка.Тумблер
	КонецЕсли;
...
Процедура ЗаполнитьИсходноеЗначение()
	...
	Значение = предопределенныеНастройки.ЗначениеПредопределеннойНастройки(Объект.ИмяНастройки, Ложь);
