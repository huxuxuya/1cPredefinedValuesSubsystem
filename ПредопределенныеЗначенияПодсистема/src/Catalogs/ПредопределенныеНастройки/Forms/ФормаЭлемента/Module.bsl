...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.ТаблицаЗначений") Тогда
		...
		ТекущийОбъект.Хранилище = Новый ХранилищеЗначения(РеквизитФормыВЗначение("ТаблицаНастройки"));
...
Процедура ПередЗаписьюНаСервереСоставнойПараметр(ТекущийОбъект)
	...
	Если ТекущийОбъект.ЭтоНовый() Тогда
		СтруктураХранилищаРанее = Новый Соответствие;
	Иначе
		СтруктураХранилищаРанее = ПредопределенныеНастройки.ЗначениеПредопределеннойНастройки(ТекущийОбъект.Ссылка, Ложь);
	КонецЕсли;
	
	Соответствие = Новый Соответствие;
	Для Каждого Строка Из ЭтаФорма.ТаблицаНастройки Цикл
		Соответствие.Вставить(Строка.Ключ, Строка.Значение);
	КонецЦикла;
	
	ТекущийОбъект.Хранилище = Новый ХранилищеЗначения(Соответствие, Новый СжатиеДанных());
...
Процедура ОтобразитьСоставнойПараметр()
	...
	ИначеЕсли Объект.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КлючЗначение") Тогда
		...
		Для Каждого Элемент Из ЗначениеХранилища Цикл
			НоваяСтрока = ТаблицаЗначений.Добавить();
			НоваяСтрока.Ключ = Элемент.Ключ;
			НоваяСтрока.Значение = Элемент.Значение;
		КонецЦикла;
		
		ОтобразитьТаблицуЗначения(ТаблицаЗначений);
...
&НаСервере
Процедура УстановитьПодсказкуПрограммногоДобавления()
	...
	// Создаем подсказку в виде текста, как шаблон для программного описания добавления элемента
	ШаблонПодсказки = НСтр("ru = 'Новые элементы справочника должны создаваться через программыный интерфейс: 
						   | ПредопределенныеНастройкиПереопределяемый.ПриЗаполненииТаблицыПредопределенныхНастроек
						   |
						   | Пример для строкового парметра:
						   |
						   |	НоваяНастройка = ТаблицаНастроек.Добавить();
						   |	НоваяНастройка.Идентификатор = Новый УникальныйИдентификатор(""%1"");
						   |	НоваяНастройка.Наименование = НСтр(""ru =''<Наименование настройки>''"");
						   |	НоваяНастройка.ИмяНастройки = ""<ИмяНастройки>"";
						   |	НоваяНастройка.ТипЗначения = ПредопределенноеЗначение(""Перечисление.ТипыЗначенияПредопределенныхЗначений.Строка"");
						   |	НоваяНастройка.Описание = НСтр(""ru =''<Описание настройки>''"");
						   |	НоваяНастройка.Значение	= ""Значение"";
						   |
						   | Идентификатор уже сгенерирован.  Просто скопируй и вставь в метод ""ПриЗаполненииТаблицыПредопределенныхНастроек"".'");
	Подсказка = СтрШаблон(ШаблонПодсказки,  СокрЛП(Идентификатор));
	Объект.Описание = Подсказка;
