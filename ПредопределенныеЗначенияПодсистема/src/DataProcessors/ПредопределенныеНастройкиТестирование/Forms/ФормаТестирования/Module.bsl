...
&НаКлиенте
Процедура СозданиеПредопределеннойНастройкиКаталогПредопределенныхНастроек(Команда)
	СозданиеПредопределеннойНастройкиКаталогПредопределенныхНастроекНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура УдалениеПредопределеннойНастройкиКаталогПредопределенныхНастроек(Команда)
	УдалениеПредопределеннойНастройкиКаталогПредопределенныхНастроекНаСервере();
КонецПроцедуры
#КонецОбласти
...
Функция ЗначениеНастройкиСоответствуетЗначению(ИмяНастройки, Знач ТипЗначения, Значение)
	...
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.ТаблицаЗначений") Тогда 
		
		ЗначенияИдентичны =  Не КоллекцияРазличаются(Значение, ПредопределенныеНастройки.Значение(ИмяНастройки)); 
		
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КлючЗначение") Тогда
		
		ЗначенияИдентичны = СоответствияИдентичны(Значение, ПредопределенныеНастройки.Значение(ИмяНастройки));
...
Функция СоответствияИдентичны(Соответствие1, Соответствие2)
	Если Соответствие1.Количество() <> Соответствие2.Количество() Тогда
		Возврат Ложь
	КонецЕсли;
	Для Каждого Элемент Из Соответствие1 Цикл
		Если Соответствие2.Получить(Элемент.Ключ) <> Элемент.Значение Тогда
			Возврат Ложь
		КонецЕсли;
	КонецЦикла;
	Для Каждого Элемент Из Соответствие2 Цикл
		Если Соответствие1.Получить(Элемент.Ключ) <> Элемент.Значение Тогда
			Возврат Ложь
		КонецЕсли;
	КонецЦикла;
	Возврат Истина
...
Функция ИменаКнопокУдаленияПоТипамЗначений()
	...
	ИменаКнопокПоТипуНастроек.Вставить(ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КаталогПредопределенныхНастроек"), "УдалениеПредопределеннойНастройкиКаталогПредопределенныхНастроек");
	Возврат ИменаКнопокПоТипуНастроек
...
&НаСервере
Процедура ДобавитьТестовуюГруппу(ТаблицаНастроек)

	НоваяНастройка = ТаблицаНастроек.Добавить();
	НоваяНастройка.Идентификатор = Новый УникальныйИдентификатор("63728614-6322-466a-af5c-a41e5ca55436");
	НоваяНастройка.ИмяНастройки = ИмяНастройкиТестовойГруппы();
	НоваяНастройка.Наименование = НСтр("ru ='Группа настроек проверки тестирования'");
...
Процедура ДобавитьТестовуюСтроку(ТаблицаНастроек)
	...
	НоваяНастройка = ТаблицаНастроек.Добавить();
	НоваяНастройка.ИмяНастройкиРодителя = ИмяНастройкиТестовойГруппы();
...
Процедура ИзменитьТестовыйКлючЗначение(ТаблицаНастроек)
	...
	Соответствие.Вставить("Ключ5", "Значение1");
	Соответствие.Вставить("Ключ6", "Значение2");
	Соответствие.Вставить("Ключ7", "Значение3");
	
	СуществующаяНастройка = ТаблицаНастроек.Найти(ИмяНастройкиТестовыйКлючЗначение(), "ИмяНастройки");
	Если СуществующаяНастройка <> Неопределено Тогда
		СуществующаяНастройка.Значение = Соответствие;
		СуществующаяНастройка.Хранилище = Новый ХранилищеЗначения(Соответствие);;
	КонецЕсли;
...
Процедура ДобавитьТестовыйКлючЗначение(ТаблицаНастроек)
	...
	Соответствие.Вставить("Ключ3", "Значение3");
	...
	//НоваяНастройка.Хранилище = Новый ХранилищеЗначения(ТаблицаЗначенийПример);
	НоваяНастройка.Наименование = "Соответствие";
	НоваяНастройка.Значение = Соответствие;
...
Процедура ПроверитьУдаленныеНастройки()
	...
	Для Каждого Строка Из ТаблицаНастроек Цикл
		...
		ТипЗначения = ПредопределенныеНастройки.ТипЗначенияПредопределеннойНастройки(Строка.Значение);
...
Процедура УдалитьНастройкуИзКоманды(ИмяНастройки)
	...
	ТипЗначения = ПредопределеннаяНастройка.ТипЗначения;

	ТаблицаНастроек = ПредопределенныеНастройки.НоваяТаблицаПредопределенныхНастроек();
	ДобавитьТестовуюНастройкуПоТипуЗначения(ТаблицаНастроек, ТипЗначения);
	Для Каждого Элемент Из ТаблицаНастроек Цикл
		Элемент.Удалена = Истина;
	КонецЦикла;
	ПредопределенныеНастройки.ОбновитьНастройкиВБазе(ТаблицаНастроек);
...
&НаСервере
Функция ИмяНастройкиТестовойГруппы()
	Возврат "Группа тестовых настроек";
КонецФункции
...
&НаСервере
Процедура СозданиеПредопределеннойНастройкиКаталогПредопределенныхНастроекНаСервере()
	
	ТаблицаНастроек = ПредопределенныеНастройки.НоваяТаблицаПредопределенныхНастроек();
	ТипНастройки = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КаталогПредопределенныхНастроек");
	ДобавитьТестовуюНастройкуПоТипуЗначения(ТаблицаНастроек, ТипНастройки);
	ПредопределенныеНастройки.ОбновитьНастройкиВБазе(ТаблицаНастроек);
...
Процедура ДобавитьТестовуюНастройкуПоТипуЗначения(ТаблицаНастроек, ТипЗначения)
	
	Если ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.Массив") Тогда
		ДобавитьТестовыйМассив(ТаблицаНастроек);
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.Строка") Тогда
		ДобавитьТестовуюСтроку(ТаблицаНастроек);
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.Булево") Тогда
		ДобавитьТестовоеБулево(ТаблицаНастроек);
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.Ссылка") Тогда
		ДобавитьТестовуюСсылку(ТаблицаНастроек);		
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КлючЗначение") Тогда
		ДобавитьТестовыйКлючЗначение(ТаблицаНастроек);
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.ТаблицаЗначений") Тогда
		ДобавитьТестовуюТаблицуЗначений(ТаблицаНастроек);
	ИначеЕсли ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КаталогПредопределенныхНастроек") Тогда
		ДобавитьТестовуюГруппу(ТаблицаНастроек);
	КонецЕсли;
...
Процедура УстановитьЗначениеПредопределеннойНастройкиКлючЗначениеНаСервере()
	...
	Соответствие = Новый Соответствие;
	Соответствие.Вставить("Ключ6", "Значение1");
	Соответствие.Вставить("Ключ9", "Значение2");
	Соответствие.Вставить("Ключ3", "Значение3");
	...
	ТаблицаНастроек[0].Хранилище = Новый ХранилищеЗначения(Соответствие);
...
&НаСервере
Процедура УдалениеПредопределеннойНастройкиКаталогПредопределенныхНастроекНаСервере()

	УдалитьНастройкуИзКоманды(ИмяНастройкиТестовойГруппы());
