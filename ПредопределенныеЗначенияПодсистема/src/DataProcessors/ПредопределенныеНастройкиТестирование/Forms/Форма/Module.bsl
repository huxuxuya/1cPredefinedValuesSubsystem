...
Процедура ПроверитьСозданныеНастройки(ТаблицаНастроек)
	...
	Для Каждого Строка Из ТаблицаНастроек Цикл
		ПроверитьЗначениеНастройки(Строка.ИмяНастройки, Строка.ТипЗначения, Строка.Значение);
...
&НаСервере
Процедура ПроверитьЗначениеНастройки(ИмяНастройки, ТипЗначения, ЗначениеИсходное)
	...
	ЗначенияИдентичны = ЗначениеНастройкиСоответствуетЗначению(ИмяНастройки, ТипЗначения, Значение);
	
	Если ЗначенияИдентичны Тогда
			УстановитьСтатусКнопки(Кнопка, Команда, Истина, НСтр("ru = 'Значение настройки проверено'"));
	Иначе
			УстановитьСтатусКнопки(Кнопка, Команда, Ложь, НСтр("ru = 'Значение настройки не совпадает'"));
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьТестовоеБулево(ИмяНастройки, ПроверяемоеЗначение, ИмяКнопки)
	
	ЗначениеВБазе = ПредопределенныеНастройки.Значение(ИмяНастройки);
	
	ТипНастройки = ПредопределенныеНастройки.ТипЗначенияПредопределеннойНастройки(ЗначениеВБазе);
	
	ЗначенияИдентичны = ЗначениеНастройкиСоответствуетЗначению(ИмяНастройки, ТипНастройки, ПроверяемоеЗначение);
