...
// Функция тестового заполнения таблицы предопределенных настроек типом значения булево
Функция ЗаполнитьТаблицуПредопределенныхНастроекТестовымиДаннымиБулево(ТаблицаНастроек)

	НоваяСтрока = ТаблицаНастроек.Добавить();
	НоваяСтрока.ИмяНастройки = "ПроверкаБезопасности";
	НоваяСтрока.Хранилище = ValueStorage.Global;
	НоваяСтрока.Содержание = "Включить проверку безопасности при входе в систему";
	НоваяСтрока.Описание = "Эта настройка позволяет включать или отключать проверку безопасности.";
	НоваяСтрока.ТипЗначения = Справочники.ПредопределенныеНастройки.НайтиПоКоду("Булево");
