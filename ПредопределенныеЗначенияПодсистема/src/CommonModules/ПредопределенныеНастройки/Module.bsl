...
//     ИмяНастройкиРодителя - Строка - имя настройки ролдителя.
...
//     Удалена - Булево - признак удаления настройки.
Функция НоваяТаблицаПредопределенныхНастроек() Экспорт

    ТаблицаНастроек = Новый ТаблицаЗначений();
    ТаблицаНастроек.Колонки.Добавить("ИмяНастройкиРодителя", Новый ОписаниеТипов("Строка"));
    ...
    ТаблицаНастроек.Колонки.Добавить("Удалена", Новый ОписаниеТипов("Булево"));
...
Процедура ОбновитьНастройкиВБазе(ТаблицаНастроекВБазе) Экспорт
	...
	Для Каждого СтрокаНастройки Из ТаблицаНастроекВБазе Цикл
		...
		НайденныйРодитель = Справочники.ПредопределенныеНастройки.ПустаяСсылка();
		Если ЗначениеЗаполнено(СтрокаНастройки.ИмяНастройкиРодителя) Тогда
			НайденныйРодитель = Справочники.ПредопределенныеНастройки.НайтиПоРеквизиту("ИмяНастройки", СтрокаНастройки.ИмяНастройкиРодителя);
		КонецЕсли;
		...
		Если СтрокаНастройки.Удалена Тогда
			Если НайденныйЭлемент <> Справочники.ПредопределенныеНастройки.ПустаяСсылка()
				И СсылкаСуществует(НайденныйЭлемент) Тогда
					
				МассивСсылок = Новый Массив;
				МассивСсылок.Добавить(НайденныйЭлемент);

				НайденныеСсылки = НайтиПоСсылкам(МассивСсылок);
				Если НайденныеСсылки.Количество() > 0 Тогда			
					Шаблон = НСтр("ru = 'Настройка ""%1"" не может быть удалена, так как она используется в других настройках.'");
					ТекстСообщение = СтрШаблон(Шаблон, СтрокаНастройки.ИмяНастройки);
					ВызватьИсключение ТекстСообщение;
				КонецЕсли;
				 
				ОбъектНастройки = НайденныйЭлемент.ПолучитьОбъект();
		...
		ИначеЕсли СтрокаНастройки.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КаталогПредопределенныхНастроек") Тогда
			
			Элемент = Справочники.ПредопределенныеНастройки.СоздатьГруппу();
			...
			Элемент.Родитель = НайденныйРодитель;
			Элемент.Записать();
			Продолжить;
		Иначе
