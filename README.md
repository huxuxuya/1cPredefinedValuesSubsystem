...
- **Права доступа**: Гибкая система доступа через роли

## Почему эта подсистема?
...
sequenceDiagram
    ...
    Note over API: Вызов ПриЗаполненииТаблицыПредопределенныхНастроек
...
Процедура ПриЗаполненииТаблицыПредопределенныхНастроек(ТаблицаНастроек) Экспорт
	...
	НоваяНастройка.Хранилище = Новый ХранилищеЗначения(МассивПользователей);
...
### Установка через EDT (рекомендуется для разработки)

#### Шаг 1: Подготовка

1. Создайте резервную копию вашей конфигурации
2. Убедитесь, что используете EDT версии, совместимой с 1С:8.3.25+
...
2. Правой кнопкой мыши на конфигурации → Compare With → Another Configuration Project
...
4. В окне "Сравнение, объединение конфигураций":
   - Установите режим "Поддержка"
   - Установите направление "← Основная конфигурация"
   - Просмотрите список добавляемых объектов:
     * Подсистема "СервисныеПодсистемы/ПредопределенныеЗначения"
     * Справочник "ПредопределенныеНастройки" (с формами)
     * Общие модули (3 штуки)
     * Перечисление "ТипыЗначенияПредопределенныхЗначений"
     * Роли (2 штуки)
     * Обработка "ПредопределенныеНастройкиТестирование"
     * Общая картинка "ТипыЗначений"
...
**ВАЖНО!** После объединения конфигурация автоматически ставится на поддержку:

1. Конфигурация → Поддержка → Настройка поддержки
2. Проверьте, что конфигурация находится на поддержке
3. Поставщик: укажите себя или оставьте пустым
...
1. Откройте справочник "Профили групп доступа"
2. Для профиля "Администратор":
   - Добавьте роль "ДобавлениеИзменениеПредопределенныеЗначения"
3. Для профиля "Пользователь":
   - Добавьте роль "ЧтениеПредопределенныеЗначения"
...
НоваяНастройка.Хранилище = Новый ХранилищеЗначения(МассивСтатусов);
```
...
**Назначение:** Пары ключ-значение для настроек
...
НоваяНастройка.Хранилище = Новый ХранилищеЗначения(СоответствиеНастроек);
```
...
**Важно:** Используйте `Хранилище` для таблиц значений!
...
НоваяНастройка.Хранилище = Новый ХранилищеЗначения(ТаблицаПравил);
```
...
Процедура ПриЗаполненииТаблицыПредопределенныхНастроек(ТаблицаНастроек) Экспорт
	...
	НоваяНастройка.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.ТаблицаЗначений");
	НоваяНастройка.Хранилище = Новый ХранилищеЗначения(ТаблицаЗначений);
...
### Сценарий 3: Настройки ролей и разрешений
...
Процедура ПриЗаполненииТаблицыПредопределенныхНастроек(ТаблицаНастроек) Экспорт
	...
	НоваяНастройка.ТипЗначения = ПредопределенноеЗначение("Перечисление.ТипыЗначенияПредопределенныхЗначений.КлючЗначение");
	НоваяНастройка.Хранилище = Новый ХранилищеЗначения(СоответствиеПрав);
...
### Сценарий 4: Настройки печатных форм
...
### Обновление настроек при изменении конфигурации
...
### Миграция значений между типами

Изменение типа настройки:

```bsl
Процедура МигрироватьНастройку()
	
	СтароеЗначение = ПредопределенныеНастройки.Значение("СтараяНастройка");
	
	// Преобразование в новый тип
	НовоеЗначение = ПреобразоватьВНовыйТип(СтароеЗначение);
	
	ТаблицаНастроек = ПредопределенныеНастройки.НоваяТаблицаПредопределенныхНастроек();
	НоваяСтрока = ТаблицаНастроек.Добавить();
	НоваяСтрока.ИмяНастройки = "НоваяНастройка";
	НоваяСтрока.НовыйТип = ОпределитьТип(НовоеЗначение);
...
Перед удалением настройки проверьте зависимости:

```bsl
Процедура УдалитьНастройкуСПроверкой(ИмяНастройки)
	
	Настройка = ПредопределенныеНастройки.НастройкаПоИмени(ИмяНастройки);
	
	// Проверка использования
	Если ИспользуетсяВДругихНастройках(Настройка) Тогда
		ВызватьИсключение "Настройка используется в других настройках!";
	КонецЕсли;
	
	ТаблицаНастроек = ПредопределенныеНастройки.НоваяТаблицаПредопределенныхНастроек();
	Строка = ТаблицаНастроек.Добавить();
	Строка.ИмяНастройки = ИмяНастройки;
	Строка.Удалена = Истина;
...
- ✅ **Зеленый индикатор** - операция выполнена успешно
- ⚠️ **Желтый индикатор** - операция в процессе
- ❌ **Красный индикатор** - ошибка при выполнении

## Решение проблем

### Настройка не найдена

**Проблема:** Получение значения настройки возвращает пустую ссылку
...
### Ошибки при обновлении настроек

**Проблема:** Ошибка при обновлении настроек в базе
...
Исключение
	...
	ВызватьИсключение СтрШаблон("Ошибка обновления настроек: %1", ТекстОшибки);
КонецПопытки;
```

### Проблемы с хранилищем значений

**Проблема:** Значения в хранилище не сохраняются

**Решение:**
```bsl
// Для массивов и таблиц всегда используйте Хранилище
Для Массива:
	НоваяНастройка.Хранилище = Новый ХранилищеЗначения(МассивЗначений);
...
**Проблема:** Дублирование UUID

**Решение:**
```bsl
// ✅ Всегда генерируйте новый UUID для каждой настройки
НоваяНастройка.Идентификатор = Новый УникальныйИдентификатор();

// ❌ Не используйте один и тот же UUID дважды
```
